...
fixed app >
  ...
  <
  v - layout v -
  if = "item.heading": key = "item.heading"
row align - center >
  <
  v - flex xs6 >
  <
...
v - list - group v -
    ...
    if = "item.children": key = "item.text"
...
append - icon = "" >
  ...
  /template> <
  v - list - tile v -
  for = "(child, i) in item.children": key = "i" >
  <
  v - list - tile - action v -
  if = "child.render &&child.icon": to = "child.to" >
  <
  v - icon > {
    {
      child.icon
...
class = "ml-0 pl-3" >
  ...
  /v-toolbar-title> <
  v - spacer > < /v-spacer> <
  v - btn flat block color = "white"
light @click = "dialog=true" > Search < /v-btn> <
  v - spacer > < /v-spacer> <
  v - btn icon large >
  <
  v - avatar size = "32px"
...
slider - color = "white" >
  <
  v - tab v -
  for = "(tab,index ) in tabItems": key = "index" > {
    {
      tab.title
    }
  } <
  ...
  v - flex xs12 sm6 md3 >
  <
  v - text - field label = "Block Number"
hint = "The Block number for example: 0"
v - model = "blockNumber" >
  <
  /v-text-field> <
  /v-flex> <
  v - btn color = "#7E57C2"
dark: disabled = "!valid"
@click = "getBlockData()" > Search < /v-btn> <
  /v-layout> <
  v - card >
  <
  v - card - title >
  ...
  /v-card-title> <
  v - card - text > Block Height: {
    {
      block.number
    }
  } < /v-card-text> <
  v - card - text > Time Stamp: {
    {
      (block.stringTime)
    }
  } < /v-card-text> <
  v - card - text > Transactions: {
    {
      block.transactions ? block.transactions.length : 0
    }
  } <
  /v-card-text> <
  v - card - text > Mined By: {
    {
      block.miner
    }
  } < /v-card-text> <
  v - card - text > Difficulty: {
    {
      block.difficulty
    }
  } < /v-card-text> <
  v - card - text > Total Difficulty: {
    {
  ...
  v - card - text > Gas Used: {
    {
      block.gasUsed
    }
  } < /v-card-text> <
  v - card - text > Gas Limit: {
    {
      block.gasLimit
    }
  } < /v-card-text> <
...
v - model = "transactionHash" >
  ...
  v - btn color = "#7E57C2"
dark: disabled = "!valid"
@click = "getTransactionData()" > Search < /v-btn> <
  /v-layout> <
  ...
  /v-card-text> <
  v - card - text > Block Hash: {
    {
      transaction.blockHash
    }
  } < /v-card-text> <
  v - card - text > BlockNumber: {
    {
  ...
  v - card - text > To: {
    {
      transaction.to ? transaction.to : "Contract creation"
    }
  } < /v-card-text> <
  v - card - text > Value: {
    {
  ...
  v - card - text > Gas Price: {
    {
      transaction.gasPrice
    }
  } < /v-card-text> <
  v - card - text > Contract Address: {
    {
      transaction.contractAddress
    }
  } < /v-card-text> <
  v - card - text > Cummulative Gas Used: {
    {
      transaction.gasPrice
    }
  } < /v-card-text> <
  v - card - text > Transaction Status: {
    {
      transaction.status
    }
  } < /v-card-text> <
    ...
    getTransactionData() {
      if (!this.transactionHash) {
        ...
        this.Web3.eth.getTransaction(this.transactionHash).then((transaction) => {
          ...
          This.error("And i-oop something went wrong!!")
        })
      }
    },
    updateTime() {
